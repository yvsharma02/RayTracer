namespace RayTracing
{
    public class TestScene
    {
        ...
        public TestScene()
        {
            Transfomration sphereTransform = new Transfomration(new Vector3D(0, -10, 10), new Vector3D(0, 0, 0), new Vector3D(5, 5, 5));
            ...
            RTColor sunColor = new RTColor(RTColor.MAX_INTENSITY / 2, 255, 255, 255);
            Vector3D sunDir = new Vector3D(0, -1f, -1f);

            world = new World(null, null);

            Camera cam = new Camera(cameraTransform, new Int2D(RES_X, RES_Y), new DefaultPixelShader(new Int2D(RAYS_PER_PIXEL_X, RAYS_PER_PIXEL_Y)), BOUCES, new RTColor(RTColor.MAX_INTENSITY, 123, 123, 255));

            world.SetMainCamera(cam);

            ShapeShader sphereShader = new AdvanceShapeShader(TextureLoader.Load(TEST_BALL_TEXTURE), null, 0, 0.25f, 0f);
